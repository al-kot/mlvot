...
class Tracker:
    def __init__(self, model_path, miss_max=5, iou_th=0.3, alpha=0.5, beta=0.5):
        self.trks = []
        self.nid = 1
    ...
    def update(self, dets, img):
        ...
        valid_dets = []
        for det in dets:
            feat = self.reid.extract(img, det[:4])
            if feat is not None:
        ...
        score_mat = np.zeros((n_trks, n_dets))

        for i, trk in enumerate(self.trks):
            pbox = trk.get("pred_box", trk["box"])
            trk_feat = trk["feat"]
            for j, det in enumerate(dets):
                iou = iou_calc(pbox, det[:4])
                sim = np.dot(trk_feat, det_feats[j])
                score_mat[i, j] = self.alpha * iou + self.beta * sim

        if n_trks > 0 and n_dets > 0:
            r_idx, c_idx = linear_sum_assignment(-score_mat)
        else:
            r_idx, c_idx = [], []
